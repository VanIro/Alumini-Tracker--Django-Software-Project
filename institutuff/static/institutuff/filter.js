...
function createTags_(filters_container,tag_name_list,tags_left_count,tag_id=0,fill=false,JSONobj=null){
    ...
    tag.className = "recipients_tag"//tag.name+"_tag";
    tag.id = tag_id;
    tag.onchange=choiceEventTags;
    
    let tag_label_container = document.createElement("div");
    tag_label_container.className = "tag_label_class";
    let label_container = document.createElement("div");
    label_container.className = "label_class";
...
function getSelectionsString(){
    ...
    if (j > 1) {
        ...
        alert("please choose both year and level\nif extra option is not appearing try selecting none and again the value")
    //console.log(j);
}
